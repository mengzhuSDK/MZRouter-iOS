...
static NSString * const MZ_ROUTER_WILDCARD_CHARACTER = @"~";
static NSString *mz_specialCharacters = @"/?&.";

NSString *const MZRouterParameterViewController = @"MZRouterParameterViewController";//传递的viewController参数的key
NSString *const MZRouterParameterNavigationController = @"MZRouterParameterNavigationController";//传递的navigationController参数的key
NSString *const MZRouterParameterBlock = @"MZRouterParameterBlock";//传递的回调block的key

NSString *const MZRouterParameterURL = @"MZRouterParameterURL";
NSString *const MZRouterParameterCompletion = @"MZRouterParameterCompletion";
NSString *const MZRouterParameterUserInfo = @"MZRouterParameterUserInfo";
...
@implementation MZRouter

+ (instancetype)sharedInstance
{
    static MZRouter *instance = nil;
...
{
    [self openURL:URL completion:^(id  _Nonnull result) {
    }];
}
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary * _Nullable)userInfo completion:(void (^)(id result))completion
{
    ...
    if (parameters) {
        MZRouterHandler handler = parameters[@"block"];
        if (completion) {
            parameters[MZRouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[MZRouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
+ (NSString *)generateURLWithPattern:(NSString *)pattern parameters:(NSArray *)parameters
{
    ...
    for (int i = 0; i < pattern.length; i++) {
        ...
        if ([mz_specialCharacters rangeOfString:character].location != NSNotFound && i > (startIndexOfColon + 1) && startIndexOfColon) {
...
{
    MZRouter *router = [MZRouter sharedInstance];
    
    URL = [URL stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    NSMutableDictionary *parameters = [router extractParametersFromURL:URL matchExactly:NO];
    MZRouterObjectHandler handler = parameters[@"block"];
...
- (void)addURLPattern:(NSString *)URLPattern andHandler:(MZRouterHandler)handler
{
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(MZRouterObjectHandler)handler
{
    NSMutableDictionary *subRoutes = [self addURLPattern:URLPattern];
    if (handler && subRoutes) {
        subRoutes[@"_"] = [handler copy];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url matchExactly:(BOOL)exactly
{
    ...
    parameters[MZRouterParameterURL] = url;
    ...
    for (NSString* pathComponent in pathComponents) {
        ...
        for (NSString* key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:MZ_ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
                ...
                if ([self.class checkIfContainsSpecialCharacter:key]) {
                    NSCharacterSet *specialCharacterSet = [NSCharacterSet characterSetWithCharactersInString:mz_specialCharacters];
...
- (NSArray*)pathComponentsFromURL:(NSString*)URL
{
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        if (!URL.length) {
            [pathComponents addObject:MZ_ROUTER_WILDCARD_CHARACTER];
        }
    }
...
+ (BOOL)checkIfContainsSpecialCharacter:(NSString *)checkedString {
    NSCharacterSet *specialCharactersSet = [NSCharacterSet characterSetWithCharactersInString:mz_specialCharacters];
